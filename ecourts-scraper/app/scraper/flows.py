...
from app.scraper.session import ScraperSession, STATE_INIT, STATE_CAPTCHA_REQUIRED, STATE_CAPTCHA_SUBMITTED, STATE_SEARCH_SUBMITTED, STATE_HISTORY_FETCHED, STATE_FAILED, STATE_COMPLETED
from app.scraper.client import ECourtsClient
from app.scraper.session import ScraperSession, STATE_INIT, STATE_CAPTCHA_REQUIRED, STATE_CAPTCHA_SUBMITTED, STATE_SEARCH_SUBMITTED, STATE_HISTORY_FETCHED, STATE_FAILED, STATE_COMPLETED, STATE_CASE_LIST_LOADED
import re
from bs4 import BeautifulSoup
from app.scraper.client import ECourtsClient
from app.scraper.processor import sanitize_html, extract_css_links, parse_full_case_data, clean_text
from app.scraper.transformer import transform_to_schema
from app.scraper.errors import TokenError, CaptchaError, RetryableError
...
async def get_case_list(session_id: str):
    ...
    session = await ScraperSession.get(session_id)
    if session.state != STATE_CASE_LIST_LOADED:
...
async def select_case(session_id: str, case_index: int):
    ...
    print(f"DEBUG: select_case fetching history for CNR {args[1]}...")
    ...
    if html_content:
        ...
        session.state = STATE_SEARCH_SUBMITTED
        await session.save()
        return {"status": "success", "cnr": args[1]}
    else:
