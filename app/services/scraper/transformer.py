...
from app.services.scraper.structs import (
...
def transform_to_schema(scraped_data: Dict[str, Any], cino: str) -> CaseSchema:
    ...
    # If case_status_text already contains disposal info (e.g. "Case disposed - Contested--ORDER")
    # we should try to extract it if not explicitly provided
    if case_status_text and "disposed" in case_status_text.lower() and "-" in case_status_text:
         parts = case_status_text.split("-", 1)
         if len(parts) > 1 and not nature_of_disposal:
